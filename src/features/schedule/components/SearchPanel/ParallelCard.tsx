...
import { hasTimeConflict, hasExamConflict, getSubjectColor, secondsToTime } from '../../../../domain/scheduler/conflicts'
import type { ParallelUnit } from './SubjectList'
import type { SubjectResult, ParallelDetail, CourseInfo } from '../../../../domain/scheduler/types'
...
function ParallelDetailSection({
  title,
  titleClassName,
  dayClassName,
  teacher,
  ...
  dayClassName: string
  teacher: string
  schedule: { nombredia: string; horainicio: number; horafin: number; aula: string }[]
}) {
  return (
        ...
        {schedule.length > 0 ? (
          schedule.map((s, i) => (
            <div key={i} className="flex items-start gap-3 justify-between bg-zinc-800/30 p-2 rounded-lg border border-zinc-700/50">
              <span className={`text-xs font-extrabold w-8 ${dayClassName}`}>{s.nombredia.slice(0, 3)}</span>
...
function ParallelCardHeader({
  ...
  isSelected: boolean
  isSubjectAlreadySelected: boolean
  hasConflictSchedule: boolean
  hasConflictExam: boolean
  expanded: boolean
  onExpand: () => void
}) {
  return (
    ...
    >
      <div className="flex gap-1.5 pt-0.5">
        {hasTeorico && <Pill variant="typeBlue">T</Pill>}
        {hasPractico && <Pill variant="typeGreen">P</Pill>}
      </div>
      <span className="text-sm font-bold text-zinc-100 truncate">{title}</span>

      {loading && cuposDisponibles === null && (
      ...
      <div className="ml-auto flex items-center gap-2">
        {isSelected ? (
          <Pill variant="success">✓ Agregado</Pill>
        ) : isSubjectAlreadySelected ? (
          <Pill variant="warning">Ya en horario</Pill>
        ) : (
          <div className="flex gap-1">
            {hasConflictSchedule && <Pill variant="danger">⚠ CRUCE HORA</Pill>}
            {!hasConflictSchedule && hasConflictExam && <Pill variant="examWarning">⚠ CRUCE EXAMEN</Pill>}
          </div>
        )}
...
function ConflictNotices({
  ...
  isSubjectAlreadySelected: boolean
  conflictSchedule: { subjectName: string; paralelo: number } | null
  conflictExam: { subjectName: string; paralelo: number } | null
}) {
  return (
    <>
      {isSubjectAlreadySelected && (
...
function ParallelActions({
  ...
  return (
    <div className="flex gap-2 pt-2">
      {!isSelected ? (
        <Button onClick={onAdd} disabled={hasConflict || !isReady || isSubjectAlreadySelected} variant="primary">
          Agregar al horario
        </Button>
      ) : (
...
export function ParallelCard({ unit, subjectIndex }: Props) {
  ...
  return (
    ...
    >
      <ParallelCardHeader
        hasTeorico={Boolean(t)}
        hasPractico={Boolean(p)}
        title={title}
        loading={Boolean(loading)}
        cuposDisponibles={tDetail?.cuposDisponibles ?? pDetail?.cuposDisponibles ?? null}
        isSelected={isSelected}
        isSubjectAlreadySelected={isSubjectAlreadySelected}
        hasConflictSchedule={Boolean(conflictSchedule)}
        hasConflictExam={Boolean(conflictExam)}
        expanded={expanded}
          ...
          {!loading && (
            <>
              ...
              {t && tDetail && !tDetail.error && (
                <ParallelDetailSection
                  title="Teórico"
                  titleClassName="text-blue-400 font-bold tracking-tight"
                  dayClassName="text-blue-400"
                  teacher={tDetail.info?.nombre_profesor || 'Sin profesor'}
                  schedule={tDetail.schedule}
                />
              )}
              ...
              {p && pDetail && !pDetail.error && (
                <ParallelDetailSection
                  title="Práctico"
                  titleClassName="text-emerald-400 font-bold tracking-tight"
                  dayClassName="text-emerald-400"
                  teacher={pDetail.info?.nombre_profesor || 'Sin profesor'}
                  schedule={pDetail.schedule}
                />
              )}
              ...
              <ConflictNotices
                isSubjectAlreadySelected={isSubjectAlreadySelected}
                conflictSchedule={conflictSchedule ? { subjectName: conflictSchedule.subjectName, paralelo: conflictSchedule.paralelo } : null}
                conflictExam={conflictExam ? { subjectName: conflictExam.subjectName, paralelo: conflictExam.paralelo } : null}
              />
