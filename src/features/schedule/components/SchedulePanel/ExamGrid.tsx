...
import { SLOT_SECONDS, secondsToTime, SUBJECT_COLORS } from '../../../../domain/scheduler/conflicts'
import { LinearScheduleGrid } from './LinearScheduleGrid'
import { buildSubjectColorMap, computeDynamicRange, EXAM_DAYS } from '../../utils/gridHelpers'
...
export function ExamGrid() {
  ...
  const examRange = useMemo(() => {
    const allExams = selected.flatMap((parallel) =>
      parallel.exams
        .filter((exam) => exam.horainicio !== undefined && exam.horafin !== undefined)
        .map((exam) => ({ horainicio: exam.horainicio, horafin: exam.horafin }))
    )
    return computeDynamicRange(allExams)
  }, [selected])
  ...
  const items = exams
    ...
    .map((b, i) => {
      ...
      const locationBadges: Array<{ label: string; kind: 'room' }> = []
      if (b.aula) {
        locationBadges.push({ label: b.aula as string, kind: 'room' })
      }
